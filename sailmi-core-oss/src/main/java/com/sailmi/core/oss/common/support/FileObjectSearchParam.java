...
public class FileObjectSearchParam implements FileObjectSearchRequest {
    ...
    public Pageable toPageable() {
        if (StringUtils.isEmpty(this.sortExpr)) {
            return PageRequest.of(this.start, this.limit);
        }
        ...
        Sort.Direction sortDirection = Sort.Direction.fromString(splitSortExpr[1]);
        if (sortDirection == null) {
            return PageRequest.of(this.start, this.limit);
        }

        return PageRequest.of(this.start, this.limit, Sort.by(sortDirection, sortField));
    }
