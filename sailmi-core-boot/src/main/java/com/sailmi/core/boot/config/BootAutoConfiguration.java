...
public class BootAutoConfiguration {
	...
	public SystemConstant fileConst() {
		...
		//设定文件上传是否为远程模式
		me.setRemoteMode(launchProperties.getBoolean("remote-mode", true));

		//远程上传地址
		me.setRemotePath(launchProperties.get("remote-path", System.getProperty("user.dir") + "/work/upload"));
		...
		//设定上传图片是否压缩
		me.setCompress(launchProperties.getBoolean("compress", false));

		//设定上传图片压缩比例
		me.setCompressScale(launchProperties.getDouble("compress-scale", 2.00));

		//设定上传图片缩放选择:true放大;false缩小
		me.setCompressFlag(launchProperties.getBoolean("compress-flag", false));
