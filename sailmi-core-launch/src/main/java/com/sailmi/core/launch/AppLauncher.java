 ...
 */
package com.sailmi.core.launch;
...
public class AppLauncher {
	...
	public static SpringApplicationBuilder createSpringApplicationBuilder(String appName, Class source, String... args) {
		...
		// 预设的环境
		List<String> presetProfiles = new ArrayList<>(Arrays.asList(LaunchConstant.DEV_CODE, LaunchConstant.TEST_CODE, LaunchConstant.PROD_CODE));
		// 交集
		...
		if (activeProfileList.isEmpty()) {
			// 默认dev开发
			profile = LaunchConstant.DEV_CODE;
		...
		String startJarPath = AppLauncher.class.getResource("/").getPath().split("!")[0];
		...
		//初始化配置，bootstrap.yml的配置将会覆盖当前配置
		...
		System.out.println("缺省配置如下：");
		Enumeration proEnum = props.propertyNames();
		while(proEnum.hasMoreElements()){
			String proKey=proEnum.nextElement().toString();
			System.out.println(proKey+":"+props.getProperty(proKey));
		}
	...
	public static boolean isLocalDev() {
		...
		return StringUtils.hasText(osName) && !(LaunchConstant.OS_NAME_LINUX.equals(osName.toUpperCase()));
	}
