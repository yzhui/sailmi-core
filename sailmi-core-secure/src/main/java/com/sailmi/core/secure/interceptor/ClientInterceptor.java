...
public class ClientInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
		...
		if (user != null && StringUtil.equals(clientId, SecureUtil.getClientIdFromHeader()) && StringUtil.equals(clientId, user.getClientId())) {
			log.info("客户端认证成功，请求接口：{}，请求IP：{}，请求参数：{}，客户端ID:{}，用户所属客户端ID：{}，客户端加密串：{}",request.getRequestURI(), WebUtil.getIP(request), JsonUtil.toJson(request.getParameterMap()),clientId,user.getClientId(),"");
			return true;
		} else {
			log.warn("客户端认证失败，请求接口：{}，请求IP：{}，请求参数：{}，客户端ID:{}，用户所属客户端ID：{}，客户端加密串：{}",request.getRequestURI(), WebUtil.getIP(request), JsonUtil.toJson(request.getParameterMap()),clientId,user.getClientId(),"");
