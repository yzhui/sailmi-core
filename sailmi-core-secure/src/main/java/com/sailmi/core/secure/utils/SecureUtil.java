 ...
 */
package com.sailmi.core.secure.utils;
...
import com.sailmi.core.secure.TokenInfo;
import com.sailmi.core.secure.constant.SecureConstant;
import com.sailmi.core.secure.exception.SecureException;
import com.sailmi.core.secure.provider.IClientDetails;
...
public class SecureUtil {
	...
	public static AuthUser getUser() {
		...
		// 优先从 request 中获取
		Object AuthUser = request.getAttribute(AUTH_USER_REQUEST_ATTR);
		if (AuthUser == null) {
			AuthUser = getUser(request);
			if (AuthUser != null) {
				// 设置到 request 中
				request.setAttribute(AUTH_USER_REQUEST_ATTR, AuthUser);
			}
		}
	...
	public static AuthUser getUser(HttpServletRequest request) {
		...
		authUser.setEnterpriseId(enterpriseId);
		return authUser;
	}
	...
	public static String[] extractAndDecodeHeader() {
		...
		System.out.println("SecureUtil:extractAndDecodeHeader:header:"+header.toString());
		header = Func.toStr(header).replace(SecureConstant.BASIC_HEADER_PREFIX_EXT, SecureConstant.BASIC_HEADER_PREFIX);
		System.out.println("SecureUtil:extractAndDecodeHeader:header after replace:"+header.toString());
		...
		for(int i=0;i<base64Token.length;i++){
			System.out.print("SecureUtil:extractAndDecodeHeader:base64Token:"+base64Token[i]);
			System.out.println(" ");
		}
		...
		System.out.println("SecureUtil:extractAndDecodeHeader:token:"+token);
